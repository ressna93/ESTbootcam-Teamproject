...
const API_BASE = isLocalhost
  ? "http://localhost:3000"
  : "https://zadutest-api.vercel.app";  // ← Vercel URL로 변경하세요!
...
async function validateUsername(username) {
  return apiFetch("${API_BASE}/api/accounts/validate-username", {
    method: "POST",
...
async function validateRegistrationNumber() {
  ...
  try {
    const data = await apiFetch(
      "${API_BASE}/api/accounts/seller/validate-registration-number",
      {
        method: "POST",
...
buyerForm?.addEventListener("submit", async (e) => {
  ...
  try {
    await apiFetch("${API_BASE}/api/accounts/buyer/signup", {
      method: "POST",
...
sellerForm?.addEventListener("submit", async (e) => {
  ...
  try {
    await apiFetch("${API_BASE}/api/accounts/seller/signup", {
      method: "POST",
      body: JSON.stringify({
        username,
        password,
